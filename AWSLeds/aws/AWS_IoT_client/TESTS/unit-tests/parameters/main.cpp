...
void test_valid_params(void) {
	...
	if( test_status >= AWSIOT_CONNECTION_SUCCESS) {
		APP_INFO(("\r\n Pass valid hostname and port & establish connection to AWS Iot endpoint successfully "));
...
void test_invalid_params(void) {
	test_case(SSL_CA_PEM, strlen(SSL_CA_PEM), SSL_CLIENTKEY_PEM,
			strlen(SSL_CLIENTKEY_PEM), SSL_CLIENTCERT_PEM,
			strlen(SSL_CLIENTCERT_PEM), AWS_INVALID_HOSTNAME,
			AWS_IOT_INVALID_SECURE_PORT);
	if( test_status >= AWSIOT_CONNECTION_SUCCESS) {
		APP_INFO(
				("\r\n Pass invalid hostname and port & connection to AWS Iot endpoint should fail"));
		APP_INFO(("***Test Failed*** \r\n"));
		TEST_ASSERT(false);
	} else {
		APP_INFO(
