...
void Adafruit_GFX::printAt(uint16_t column, uint16_t row,TextAlignment_t align, uint8_t size,char *text )
{
  ...
  y = row * _height/screens[size].rows;

  switch(align)
  {
    case ALIGN_LEFT:
        x = column * _width/screens[size].cols;
      break;
    case ALIGN_RIGHT:
        x = (column-len) * _width/screens[size].cols;
      break;
    case ALIGN_ABS:
      break;
    case ALIGN_CENTER:
      x = (column-len/2) * _width/screens[size].cols;
